    ...
    with open(sim_file, "w") as sim_file_fh:
        for i, item in enumerate(genes):
            ...
            for j, item in enumerate(genes):
                ...
                down = 0
                max = dict()
                if gene[genes[i]] and gene[genes[j]]:
                    pass
                else:
                    _ = sim_file_fh.write(f"{i_apid} {j_apid} NA\n")
                    pre_file = ""
                    continue
                for a in gene[genes[i]].keys():
                    if a not in father_dict:
                        raise ValueError(f"{a} not found.")
                    a = a[3:]
                    Ta = own_set(a)
                    for b in gene[genes[i]].keys():
                        b = b[3:]
                        pair = f"{a}{b}"
                        riap = f"{b}{a}"
                        sim = 0
                ...
                down += len(gene[genes[j]])

                for key in gene[genes[i]]:
                    key = key[3:]
                    up += max[key]
                for key in gene[genes[j]]:
                    key = key[3:]
                    up += max[key]
                if down == 0:
                    gsim = 0
            ...
            _ = sim_file_fh.write(f"{i_apid} {j_apid} {gsim}\n")
        simi.append(gsim)

    if pre_file != "":
        with open(pre_file, "w") as pre_file_fh:
            sort = sorted(simi)
            index = int(len(simi) / 2)
            t = sort[index]
